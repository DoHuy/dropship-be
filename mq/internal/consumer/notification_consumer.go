...
func NewNotificationConsumer(c config.Config) *NotificationConsumer {
...
func (c *NotificationConsumer) Consume(ctx context.Context, key, val string) error {
	logx.Infof("📥 Nhận được event từ Kafka: %s", val)

	// 1. Ép chuỗi JSON thành Struct
	var payload kafka_events.OrderEventPayload
	if err := json.Unmarshal([]byte(val), &payload); err != nil {
		logx.Errorf("❌ Lỗi parse JSON event: %v", err)
	...
	default:
		logx.Infof("⚠️ Bỏ qua event không thuộc chức năng thông báo: %s", payload.EventType)
		return nil
	}
}
...
func (c *NotificationConsumer) handleOrderPaid(ctx context.Context, payload kafka_events.OrderEventPayload) error {
	...
	if err != nil {
		logx.Errorf("❌ Lỗi gửi Telegram cho đơn %s: %v", payload.OrderNumber, err)
	...
	if payload.CustomerEmail != "" {
		...
		if err != nil {
			logx.Errorf("❌ Lỗi gửi Email qua SES cho %s: %v", payload.CustomerEmail, err)
		} else {
			logx.Infof("📧 Đã gửi Email hóa đơn thành công cho khách hàng: %s", payload.CustomerEmail)
		}
	} else {
		logx.Infof("⚠️ Đơn hàng %s không có Email khách hàng, bỏ qua bước gửi mail.", payload.OrderNumber)
	}

	logx.Infof("✅ Hoàn tất xử lý Notification cho đơn hàng: %s", payload.OrderNumber)
